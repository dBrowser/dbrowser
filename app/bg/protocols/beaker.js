...
const BEAKER_CSP = `
  ...
  img-src beaker: asset: data: dweb: http: https;
  script-src 'self' beaker: 'unsafe-eval';
  media-src 'self' beaker: dweb:;
...
default-src 'self' beaker:;
img-src beaker: asset: data: dweb: http: https;
script-src 'self' beaker: dweb: blob: 'unsafe-eval';
media-src 'self' beaker: dweb:;
style-src 'self' 'unsafe-inline' beaker:;
...
async function beakerProtocol (request, respond) {
  ...
  if (requestUrl === 'beaker://dwebx-dns-cache/') {
    return cb(200, 'OK', 'text/html; charset=utf-8', datDnsCachePage)
  }
  if (requestUrl === 'beaker://dwebx-dns-cache/main.js') {
